...
<p style="text-align: justify;">
	...
	Sending data to the cloud is a common task in mobile applications. Here we will consume an open REST service to practice sending data in JSON format to a cloud server. We will use `HttpURLConnection` to make an HTTP POST request to the server. Then, we can verify that our request is successful by going to <a href="https://hmkcode-api.appspot.com/links.html">hmkcode-api.appspot.com/links.html</a> page.
